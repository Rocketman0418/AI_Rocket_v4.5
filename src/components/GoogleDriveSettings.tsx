...
import { HardDrive, CheckCircle, XCircle, Trash2, FolderOpen, RefreshCw, Info, FileText, CreditCard as Edit, Search, Sparkles, Zap, Upload } from 'lucide-react';
import {
...
export const GoogleDriveSettings: React.FC<GoogleDriveSettingsProps> = ({ fromLaunchPrep = false }) => {
  ...
  const loadSyncedDocuments = async () => {
    try {
      ...
      const { data: documents, error: docsError } = await supabase
        .rpc('get_team_documents_list', { p_team_id: teamId });

      if (docsError) throw docsError;

      const mappedDocuments = (documents || []).map((doc: any) => ({
        id: doc.google_file_id,
        source_id: doc.google_file_id,
        title: doc.file_name,
        folder_type: doc.category || 'unknown',
        created_at: doc.synced_at,
  ...
  const handleDeleteDocument = async (docId: string) => {
    ...
    try {
      ...
      const googleFileId = doc.source_id || doc.id;
      if (!googleFileId) {
        throw new Error('Document google_file_id not found. Cannot delete document.');
      }

      console.log('Deleting chunks with google_file_id:', googleFileId);
      ...
      if (chunksError) {
        console.error('Failed to delete chunks from document_chunks:', chunksError);
  ...
  return (
    <div className="bg-gray-700/50 rounded-lg p-6 border border-gray-600">
                          ...
                          >
                            {syncResult?.success ? (
                              ...
                              <>
                                ...
                                <span>{manualSyncing ? 'Syncing...' : 'Sync Folder'}</span>
                              </>
                            )}
                          </button>
                          <button
                            ...
                            <>
                              ...
                              <div className="grid grid-cols-2 gap-2 mt-2">
                                <button
                                  ...
                                  className="px-3 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm rounded transition-colors flex items-center justify-center space-x-2"
                                >
                                ...
                                <button
                                  ...
                                  className="px-3 py-2 bg-green-600 hover:bg-green-700 text-white text-sm rounded transition-colors flex items-center justify-center space-x-2"
                                >
      ...
      <GoogleOAuthInfoModal
        ...
        onProceed={handleProceedToOAuth}
      />
    </div>
  );
};
