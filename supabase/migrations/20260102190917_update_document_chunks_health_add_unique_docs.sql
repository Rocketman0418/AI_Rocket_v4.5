/*
  # Update document_chunks_health function

  1. Changes
    - Add `unique_documents` count (COUNT DISTINCT source_id)
    - `total_rows` represents total records in table
    - `active_rows` remains unchanged (used for health calculation)
*/
...
AS $function$
    SELECT 
        ...
        (SELECT count(DISTINCT source_id) FROM document_chunks WHERE source_id IS NOT NULL) as unique_documents,
