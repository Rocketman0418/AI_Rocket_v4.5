/*
  # Create Performance Metrics Function for Document Chunks

  1. New Function
    - `get_document_chunks_performance_metrics` - Returns performance indicators for the document_chunks table
    
  2. Metrics Tracked
    - Index hit rate (should be >99% for good performance)
    - Sequential scan ratio (lower is better)
    - Table bloat estimate
    - Average row size
    - Largest team stats
    - Query performance indicators
    - Partitioning recommendation based on thresholds

  3. Purpose
    - Help administrators monitor database performance
    - Provide data-driven partitioning recommendations
*/
...
BEGIN
  ...
  -- Get team count
  SELECT COUNT(DISTINCT team_id) INTO v_team_count FROM document_chunks;

  -- Determine partitioning recommendation
  -- Trigger 1: Row count > 500K
...
GRANT EXECUTE ON FUNCTION get_document_chunks_performance_metrics() TO authenticated;
