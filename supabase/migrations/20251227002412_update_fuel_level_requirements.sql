/*
  # Update Fuel Level Requirements

  1. Changes
    - Level 1: Now requires 1 document synced (removed Google Drive requirement)
    - Level 2: Now requires 5 documents and 2 categories
    - Level 3: Now requires 50 documents and 5 categories
    - Level 4: Now requires 200 documents and 8 categories
    - Level 5: Now requires 1000 documents and 12 categories

  2. Impact
    - Makes Level 1 accessible via local file uploads
    - Google Drive connection is now optional
    - Adjusted progression for more gradual advancement
*/

-- Update the fuel level calculation function with new thresholds
...
BEGIN
  ...
  -- Updated level calculation based on new requirements
  -- Level 1: 1+ document
  ...
  IF v_next_level_docs IS NOT NULL THEN
    -- Calculate based on both documents and categories progress
    DECLARE
    ...
    BEGIN
      ...
      v_progress_percentage := GREATEST(0, v_progress_percentage); -- Ensure non-negative
    END;
  ELSE
    v_progress_percentage := 100; -- Max level reached
  END IF;
