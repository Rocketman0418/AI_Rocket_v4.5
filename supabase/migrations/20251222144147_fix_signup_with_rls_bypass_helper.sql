/*
  # Fix signup with RLS bypass helper function
  
  1. Changes
    - Create a helper function that inserts/updates users bypassing RLS
    - Update main trigger to use this helper
    
  2. Security
    - Helper function is SECURITY DEFINER with explicit RLS bypass
    - Only called from the signup trigger
*/

-- Helper function to upsert user bypassing RLS
CREATE OR REPLACE FUNCTION upsert_user_bypass_rls(
  p_id uuid,
  p_email text,
  p_name text,
  p_team_id uuid,
  p_role text,
  p_view_financial boolean,
  p_is_setup_admin boolean DEFAULT false
) RETURNS void AS $$
BEGIN
  -- Direct insert/update bypassing RLS
  INSERT INTO public.users (id, email, name, team_id, role, view_financial, is_setup_admin)
  VALUES (p_id, p_email, p_name, p_team_id, p_role, p_view_financial, p_is_setup_admin)
  ON CONFLICT (id) DO UPDATE SET
    email = EXCLUDED.email,
    name = COALESCE(NULLIF(EXCLUDED.name, ''), public.users.name),
    team_id = EXCLUDED.team_id,  -- Always use the new value, don't COALESCE
    role = EXCLUDED.role,
    view_financial = EXCLUDED.view_financial,
    is_setup_admin = EXCLUDED.is_setup_admin,
    updated_at = now();
END;
$$ LANGUAGE plpgsql SECURITY DEFINER
SET search_path = public
SET row_security = off;  -- This is the key - explicitly disable RLS

-- Grant execute to authenticated and service role
GRANT EXECUTE ON FUNCTION upsert_user_bypass_rls TO service_role;
GRANT EXECUTE ON FUNCTION upsert_user_bypass_rls TO postgres;
...
BEGIN
  BEGIN
    ...
    -- Use helper function to bypass RLS
    v_error_context := 'upserting user record';
    ...
    RAISE LOG '[Signup] After upsert - user team_id is: %', v_final_team_id;
  ...
  EXCEPTION WHEN OTHERS THEN
    ...
    -- Fallback: try to create user with whatever values we have
    BEGIN
      PERFORM upsert_user_bypass_rls(NEW.id, NEW.email, v_full_name, v_team_id, v_user_role, v_view_financial, false);
    EXCEPTION WHEN OTHERS THEN
      RAISE LOG '[Signup Error] Fallback upsert also failed: %', SQLERRM;
    END;
