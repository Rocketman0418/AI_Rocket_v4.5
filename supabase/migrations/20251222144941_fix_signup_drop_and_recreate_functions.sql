/*
  # Debug signup - make failures visible
  
  Drop and recreate functions with explicit error checking.
*/

-- Drop existing functions
DROP FUNCTION IF EXISTS public.upsert_user_bypass_rls(uuid, text, text, uuid, text, boolean, boolean);

-- Recreate with explicit error checking
CREATE OR REPLACE FUNCTION public.upsert_user_bypass_rls(
  p_id uuid,
  p_email text,
  p_name text,
  p_team_id uuid,
...
DECLARE
  v_count int;
BEGIN
  RAISE LOG '[upsert_user_bypass_rls] Starting for id: %, email: %, team_id: %', p_id, p_email, p_team_id;
  ...
  RAISE LOG '[upsert_user_bypass_rls] INSERT completed, checking result';
  
  -- Verify the insert happened
  SELECT COUNT(*) INTO v_count FROM public.users WHERE id = p_id;
  RAISE LOG '[upsert_user_bypass_rls] User count after insert: %', v_count;
  
  IF v_count = 0 THEN
    RAISE EXCEPTION 'UPSERT_FAILED: User not found after insert for id: %, email: %', p_id, p_email;
  END IF;
END;
$$;

-- Update handle_new_user_signup with row_security = off
